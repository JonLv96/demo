    ...
    static class Solution {
        public int myAtoi(String s) {
            ...
            for (int i = 0; i < chars.length; i++) {
                //若"-"后非数字，或者"+"后非数字则返回0
                if (((chars[i] == 45 || chars[i] == 43) && i + 1 < chars.length && !(chars[i + 1] >= 48 && chars[i + 1] <= 57))) {
                    return 0;
                }
                ...
                //只需校验  Integer.MAX_VALUE的前214748364 即可，这样后续就不会超过int的最大值
                if (flag && (result > maxNum || (result == maxNum && chars[i] > 55))) {
                    return Integer.MAX_VALUE;
                }
                // 校验 -2³¹
                if (!flag && (result * (flag ? 1 : -1) < maxNum * -1 || (result * (flag ? 1 : -1) == maxNum * -1 && chars[i] < -8))) {
